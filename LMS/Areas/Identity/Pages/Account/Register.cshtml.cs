...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            var uIDGenerator = (from professor in db.Professors
                                select professor.UId
                                ).Union(from student in db.Students
                                        select
                                            student.UId
                                ).Union(from administrator in db.Administrators
                                        select
                                            administrator.UId
                                );
            ...
            foreach (string x in uIDGenerator) {
                int temp = Convert.ToInt32(x.Substring(1));
                maxID = Math.Max(temp, maxID);
            }
            maxID++;

            string idealUID = maxID.ToString();

            while (idealUID.Length < 7) {
                idealUID = "0" + idealUID;
            }
            idealUID = "u" + idealUID;
